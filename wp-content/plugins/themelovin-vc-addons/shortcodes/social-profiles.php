...
/* Social profiles
/*-----------------------------------------------------------------------------------*/
function thmlv_shortcode_social_profiles($atts, $content = NULL) {
	...
	foreach ($social_profiles as $service => $details) {
		if ($details['url'] !== '') {
			$output .= '<li><a href="' . esc_url($details['url']) . '" target="_blank" title="' . $details['title'] . '"><i class="fa fa-' . $service . '"></i></a></li>';
		}
	}
	
	return '<ul class="thmlv-social-profiles icon-size-' . esc_attr($icon_size) . ' align-' . esc_attr($alignment) . '">' . $output . '</ul>';
}
	
add_shortcode('thmlv_social_profiles', 'thmlv_shortcode_social_profiles');