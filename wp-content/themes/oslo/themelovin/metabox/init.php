...
if ( ! class_exists( 'CMB2_Bootstrap_209', false ) ) {
	...
	class CMB2_Bootstrap_209 {
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'oslo', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'oslo', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'oslo', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'themelovin' );
				$mofile = dirname( __FILE__ ) . '/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'oslo', $mofile );
			}
