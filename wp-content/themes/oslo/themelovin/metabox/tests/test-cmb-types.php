...
class Test_CMB2_Types extends Test_CMB2 {
					...
					<div class="cmb-tbody cmb-field-list">
						<div class="cmb-row cmb-repeat-row">
							...
							<div class="cmb-td cmb-remove-row">
								<button class="button cmb-remove-row-button button-disabled">' . __( 'Remove', 'oslo' ) . '</button>
							</div>
						</div>
						<div class="cmb-row empty-row hidden">
							...
							<div class="cmb-td cmb-remove-row">
								<button class="button cmb-remove-row-button">' . __( 'Remove', 'oslo' ) . '</button>
							</div>
						</div>
					</div>
				</div>
	...
	public function test_file_list_field() {
		$this->assertHTMLstringsAreEqual(
			'<input type="hidden" class="cmb2-upload-file cmb2-upload-list" name="field_test_field" id="field_test_field" value="" size="45" data-previewsize=\'[120,120]\' data-queryargs=\'\'/><input type="button" class="cmb2-upload-button button cmb2-upload-list" name="" id="" value="' . __( 'Add or Upload Files', 'oslo' ) . '"/><p class="cmb2-metabox-description">This is a description</p><ul id="field_test_field-status" class="cmb2-media-status cmb-attach-list"></ul>',
	...
	public function test_file_field() {
		$this->assertHTMLstringsAreEqual(
			'<input type="text" class="cmb2-upload-file regular-text" name="field_test_field" id="field_test_field" value="" size="45" data-previewsize=\'[199,199]\' data-queryargs=\'\'/><input class="cmb2-upload-button button" type="button" value="' . __( 'Add or Upload File', 'oslo' ) . '" /><p class="cmb2-metabox-description">This is a description</p><input type="hidden" class="cmb2-upload-file-id" name="field_test_field_id" id="field_test_field_id" value="0"/><div id="field_test_field_id-status" class="cmb2-media-status"></div>',
	...
	public function test_oembed_field_after_value_update() {
		...
		$src = 'http://www.youtube.com/embed/' . $vid . '?feature=oembed';
 		...
 		$results = $this->is_connected()
 			? sprintf( '<div class="embed-status"><iframe width="640" height="360" src="%s" frameborder="0" allowfullscreen></iframe><p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button" rel="field_test_field">' . __( 'Remove Embed', 'oslo' ) . '</a></p></div>', $src )
 			: sprintf( '<p class="ui-state-error-text">%2$s <a href="codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>', $value, sprintf( __( 'No oEmbed Results Found for %s. View more info at', 'oslo' ), $wp_embed->maybe_make_link( $value ) ) );
